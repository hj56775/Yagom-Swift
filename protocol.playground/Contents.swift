...
// 어떤 프로토콜의 요구사항을 모두 따르는 타입은 '그 프로토콜으르 준수한다(conform)'고 표현합니다
...
protocol Talkable {
    ...
    // get은 읽기만 가능해도 상괍없다는 뜻이며 get과 set를 모두 명시하면 읽기 쓰기 모두 가능한 프로퍼티여야 합니다
...
someAny = sub
if let someReadable : ReadSpeakable = someAny as? ReadSpeakable{
    someReadable.read()
    someReadable.speak()
}// read, speak
