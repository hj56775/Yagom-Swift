...
enum VendingMachineError : Error {
    case invalidInput
    case insufficietFunds(moneyNeeded: Int)
    case outOfStock
}
...
// 오류 발생의 여지가 있는 메서드는 throw를 사용하여 오류를 내포하는 함수임을 표시합니다
class VendingMachine {
    ...
    func vend(numberOfItems numberOfItemsToVend: Int) throws -> String {
        ...
        guard numberOfItemsToVend * itemPrice <= deposited else{
            ...
            throw VendingMachineError.insufficietFunds(moneyNeeded: moneyNeeded)
        }
...
do {
    ...
    print("입력이 잘못되었습니다")
} catch VendingMachineError.insufficietFunds(let moneyNeeded){
    print("\(moneyNeeded)원이 부족합니다")
} catch VendingMachineError.outOfStock{
    print("수량이 부족합니다")
}//  입력이 잘못되었습니다

// 하나의 catch 블럭에서 switch 구문을 사용하여 오류를 분류해봅니다
// 굳이 위의 것과 크게 다를 것아 없습니다
do {
    try machine.receiveMoney(300)
} catch/*(let error)*/{
    switch error {
    case VendingMachineError.insufficietFunds(let moneyNeeded):
        print("\(moneyNeeded)원이 부족합니다")
...
// 오류가 발생하면 러나임 오류가 발생하여 애플리케이션 동작이 중지됩니다
